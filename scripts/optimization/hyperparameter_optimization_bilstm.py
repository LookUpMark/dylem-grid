...
def main():
    ...
    print_header("BiLSTM Hyperparameter Optimization — Optuna")

    # Initialize W&B for optimization tracking
    if WANDB_AVAILABLE:
        wandb.init(
            project="dylem-grid-gestures",
            name="bilstm-optuna-optimization",
            config={
            ...
            tags=["optimization", "bilstm", "optuna"]
        )
    ...
    data, labels = data_loader('data/DYLEM-GRID', 'Cleaned')
    ...
    study = optuna.create_study(
        ...
        study_name='gesture_recognition_optimization'
    )
    ...
    best_trials_at_max_acc = [t for t in completed_trials if abs(t.value - best_accuracy) < 1e-6]
    
    # Among trials with best accuracy, choose the one with fewest parameters
    best_trial = min(best_trials_at_max_acc, key=lambda t: t.user_attrs.get('num_params', float('inf')))
    ...
    results_dir = os.path.join('results', 'optuna', 'bilstm')
    ...
    # Save best parameters to JSON inside results dir
    results = {
    ...
    json_path = os.path.join(results_dir, 'optuna_results.json')
    ...
    try:
        ...
        fig.write_image(os.path.join(results_dir, 'optuna_optimization_history.png'))
        print_success(f"Optimization history saved to {results_dir}")

        # 2. Parameter importances
        fig = plot_param_importances(study)
        fig.write_image(os.path.join(results_dir, 'optuna_param_importances.png'))
        print_success(f"Parameter importances saved to {results_dir}")

        # 3. Parallel coordinate plot
        fig = plot_parallel_coordinate(study)
        fig.write_image(os.path.join(results_dir, 'optuna_parallel_coordinate.png'))
        print_success(f"Parallel coordinate plot saved to {results_dir}")

        # 4. Slice plot
        fig = plot_slice(study)
        fig.write_image(os.path.join(results_dir, 'optuna_slice.png'))
        ...
        if WANDB_AVAILABLE:
            wandb.log({
                "optimization/history": wandb.Image(os.path.join(results_dir, 'optuna_optimization_history.png')),
                "optimization/param_importances": wandb.Image(os.path.join(results_dir, 'optuna_param_importances.png')),
                "optimization/parallel_coordinate": wandb.Image(os.path.join(results_dir, 'optuna_parallel_coordinate.png')),
                "optimization/slice": wandb.Image(os.path.join(results_dir, 'optuna_slice.png')),
            })
    ...
    print_info("1. Update train_bilstm.py with best hyperparameters")
